...
| 2 | `vol_21d` | Traditional | Rolling 21-day std of `log_return` $\times \sqrt{252}$ | Short-term (≈1 month) annualized realized volatility. |
| 3 | `vol_63d` | Traditional | Rolling 63-day std of `log_return` $\times \sqrt{252}$ | Medium-term (~3 months) annualized volatility. |
| 4 | `vol_252d` | Traditional | Rolling 252-day std of `log_return` $\times \sqrt{252}$ | Annual realized volatility; core BSM vol input. |
...
| 9 | `high_low_range_pct` | Traditional | $(\text{High} - \text{Low}) / \text{Close} \times 100$ | Intraday volatility proxy; complements close-to-close vol. |
...
| 12 | `sentiment_proxy` | Advanced | $1 - \text{minmax\_norm}(\text{VIX})$ over 252d rolling window | Placeholder for sentiment (0–1 scale); high VIX → low sentiment. Full NewsAPI/VADER later. |
...
2. **Annualization**: Volatilities use $\sqrt{252}$ (trading days per year).
